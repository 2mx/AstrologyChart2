...
const POINT_PROPERTIES_OFFSET_STEP = 1.5;
...
const MAJOR_ASPECT_ANGLES = /* @__PURE__ */ new Set([0, 60, 90, 120, 180]);
const ORBS_ASPECTS_NATAL = [
  { ...A.Conjunction, orb: 8 },
  { ...A.Opposition, orb: 8 },
  { ...A.Trine, orb: 6 },
  { ...A.Square, orb: 6 },
...
const ASPECTS_DISPLAY = "major";
const DEFAULT_ASPECTS = ORBS_ASPECTS_NATAL;
...
class SVGUtils {
  ...
  static SVGSymbol(name, xPos, yPos) {
    ...
    function quintileSymbol(xPos2, yPos2) {
      return SVGUtils.SVGText(xPos2, yPos2, SVGUtils.SYMBOL_QUINTILE_CODE);
    }
...
class RadixChart extends Chart {
  ...
  getAspects(fromPoints, toPoints, aspects) {
    ...
    const catalog = aspects ?? this.#settings.ORBS_ASPECTS_NATAL ?? SETTINGS.ORBS_ASPECTS_NATAL;
    const filter = this.#settings.ASPECTS_DISPLAY ?? SETTINGS.ASPECTS_DISPLAY ?? "major";
    const filtered = AspectUtils.filterAspects(catalog, filter);
    return AspectUtils.getAspects(fromPoints, toPoints, filtered).filter((aspect) => aspect.from.name !== aspect.to.name);
  }
  /**
   * Draw aspects
   *
...
class TransitChart extends Chart {
  ...
  getAspects(fromPoints, toPoints, aspects) {
    ...
    const catalog = aspects ?? this.#settings.ORBS_ASPECTS_TRANSIT ?? SETTINGS.ORBS_ASPECTS_TRANSIT;
    const filter = this.#settings.ASPECTS_DISPLAY ?? SETTINGS.ASPECTS_DISPLAY ?? "major";
    const filtered = AspectUtils.filterAspects(catalog, filter);
